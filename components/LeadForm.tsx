...
export function LeadForm({ services }: { services: { id: string; name: string }[] }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const serviceField = form.elements.namedItem("serviceId");
    if (serviceField instanceof HTMLSelectElement) data.serviceLabel = serviceField.selectedOptions[0]?.text ?? "";
  ...
  if (state === "success") return <QuoteSuccess reference={reference} onNewRequest={() => { setStep(1); setState("idle"); }} />;
  ...
  return <form className="lead-form" onSubmit={submit} ref={formRef}>
    ...
    {state === "error" && <p className="form-error" role="alert">Impossible d’enregistrer la demande pour le moment. Réessayez plus tard ou contactez-nous par téléphone.</p>}
  </form>;
}
